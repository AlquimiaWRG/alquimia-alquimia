 ...
 * # Example
 *
 * If your application is running at `http://localhost/app` and you want to link to `/home/products`,
...
module.exports = ['$location', ($location) => {
  let baseUrl, absoluteUrlPattern = /^(?:https?:\/\/)|(?:www)/;
  ...
  return {
    ...
    link: (scope, element) => {
      ...
      var stopWatching = scope.$watch('ngHref', value => {
  ...
  function getBaseUrl() {
    ...
    return `${$location.protocol()}://${$location.host()}:${$location.port()}${base.substring(0, base.length - 1)}`
  }
}];
